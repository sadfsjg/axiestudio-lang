...
from axiestudio.api.v1.chat import build_flow_and_stream
from axiestudio.api.v1.schemas import InputValueRequest
from axiestudio.logging import logger
from axiestudio.memory import aadd_messagetables
from axiestudio.schema.properties import Properties
from axiestudio.services.auth.utils import get_current_user_for_websocket
...
from axiestudio.services.deps import get_variable_service, session_scope
...
async def authenticate_and_get_openai_key(session: DbSession, user: User, websocket: WebSocket):
    ...
    if user is None:
        await websocket.send_json(
            {
                "type": "error",
                "code": "axiestudio_auth",
                "message": "You must pass a valid Axie Studio token or cookie.",
            }
        )
        return None, None
                    ...
                    while True:
                        ...
                        if msg.get("type") == "input_audio_buffer.append":
                            ...
                            if num_audio_samples > AUDIO_SAMPLE_THRESHOLD:
                                ...
                                num_audio_samples = 0
                        elif msg.get("type") == "axiestudio.voice_mode.config":
                            logger.info(f"axiestudio.voice_mode.config {msg}")
                            voice_config.progress_enabled = msg.get("progress_enabled", True)
                        elif msg.get("type") == "axiestudio.elevenlabs.config":
                            logger.info(f"axiestudio.elevenlabs.config {msg}")
                ...
                try:
                    while True:
                        ...
                        if event.get("type") == "input_audio_buffer.append":
                            ...
                            openai_send(event)
                        elif event.get("type") == "axiestudio.elevenlabs.config":
                            logger.info(f"axiestudio.elevenlabs.config {event}")
