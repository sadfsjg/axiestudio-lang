...
def _get_langsmith_tracer():
    from axiestudio.services.tracing.langsmith import LangSmithTracer

    return LangSmithTracer


def _get_langwatch_tracer():
    from axiestudio.services.tracing.langwatch import LangWatchTracer

    return LangWatchTracer


def _get_langfuse_tracer():
    from axiestudio.services.tracing.langfuse import LangFuseTracer

    return LangFuseTracer


def _get_arize_phoenix_tracer():
    from axiestudio.services.tracing.arize_phoenix import ArizePhoenixTracer

    return ArizePhoenixTracer


def _get_opik_tracer():
    from axiestudio.services.tracing.opik import OpikTracer
...
class TracingService(Service):
    ...
    async def start_tracers(
        ...
        if self.deactivated:
            return
        try:
            project_name = project_name or os.getenv("LANGCHAIN_PROJECT", "Axie Studio")
    ...
    def project_name(self):
        if self.deactivated:
            return os.getenv("LANGCHAIN_PROJECT", "Axie Studio")
        trace_context = trace_context_var.get()
        if trace_context is None:
