...
from axiestudio.custom.directory_reader.utils import (
...
from axiestudio.custom.schema import MissingDefault
...
from axiestudio.template.frontend_node.custom_components import ComponentFrontendNode, CustomComponentFrontendNode
from axiestudio.type_extraction.type_extraction import extract_inner_type
from axiestudio.utils import validate
from axiestudio.utils.util import get_base_classes
...
def run_build_config(
    ...
    # we have imported it
    # If not, it means the component was loaded through AXIESTUDIO_COMPONENTS_PATH
...
async def get_single_component_dict(component_type: str, component_name: str, components_paths: list[str]):
    ...
    for base_path in components_paths:
        ...
        if module_path.exists():
            ...
            module_name = f"axiestudio.components.{component_type}.{component_name}"
            try:
...
async def load_custom_component(component_name: str, components_paths: list[str]):
    ...
    """
    from axiestudio.interface.custom_component import get_custom_component_from_name
                ...
                for category_dir in base_path.iterdir():
                    if category_dir.is_dir():
                        ...
                        if component_file.exists():
                            # Try to import the module
                            module_name = f"axiestudio.components.{category_dir.name}.{component_name}"
                            try:
                                import importlib.util
