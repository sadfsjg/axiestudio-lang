...
def create_class(code, class_name):
    ...
    code = code.replace("from axiestudio import CustomComponent", "from axiestudio.custom import CustomComponent")
    code = code.replace(
...
def get_default_imports(code_string):
    ...
    axiestudio_imports = list(CUSTOM_COMPONENT_SUPPORTED_TYPES.keys())
    necessary_imports = find_names_in_code(code_string, axiestudio_imports)
    axiestudio_module = importlib.import_module("axiestudio.field_typing")
    default_imports.update({name: getattr(axiestudio_module, name) for name in necessary_imports})
